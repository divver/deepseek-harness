...
async function harness(config: Partial<ConstructorParameters<typeof CoopService>[1]> = {}): Promise<{
  ...
  const create = (id: string): Promise<Agent> => ctx.agentLoop.create(SessionId(id), { provider: 'mock', model: 'mock' }, { cwd })
  return { ctx, cwd, master: await create('master'), worker: await create('worker'), other: await create('other') }
}
...
describe('registration', () => {
  it('creates the registry on first role set and mirrors the event', async () => {
    ...
    expect(master.session.snapshotEvents().some(event => event.type === 'coop/registry')).toBe(true)
  })
  ...
  it('writes no mirror events by default (cross-build resume safety)', async () => {
    ...
    await ctx.coop.setRoles(master, { set: ['master'] })
    expect(master.session.snapshotEvents().some(event => event.type.startsWith('coop/'))).toBe(false)
  })
