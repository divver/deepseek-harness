...
export const COOP_V2_POLICY_TEXT = [
  ...
  '- **Plans** are task DAGs: `coop_plan_create` → `coop_task_add` (with `dependsOn`) → `coop_plan_submit_review`; a reviewer\'s `coop_plan_review` pass activates it (request_changes returns it to designing). Add or link tasks any time — cycles reject. Watch the board with `coop_board`.',
  '- **Tasks**: when a `[coop] task assigned` notice wakes you, call `coop_execute_begin`, do the work per the spec (or delegate it to a subagent when the task says so), touching `coop_execute_touch` on long runs, then `coop_execute_report`. Reviewers call `coop_task_verify` on the wake that follows a report; `request_changes` returns the task to its worker as rework until the budget runs out and it blocks.',
