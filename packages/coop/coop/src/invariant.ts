...
/** Install validation for newly appended coop mirrors. */
const install: InvariantInstaller = Object.assign((ctx: Context, fail: InvariantFailure) => {
  // Mirrors are observation records and the workspace-shared files stay
  // authoritative, so validation covers newly appended events only — a
  // synchronous scan of loaded session history is prohibited by the
  // deprecate-synchronous-session-event-reads decision.
