...
- **Herdr TUI 集成（P5a）** —— `coop_worker_create`（及 `/coop spawn worker|reviewer`）自动创建节点：herdr 可达且 master 位于 herdr 窗格内时，节点落在新切出的窗格中运行 `spawnCommand`，随后收到它的 `/coop <role> --master <id>` 注册行（注册时经 `HERDR_PANE_ID` 自报 pane id）；否则创建进程内 headless 会话并直接预绑定。每个 poll tick 把节点状态镜像进 herdr（`pane report-agent`/`report-metadata`，仅变化时调用），herdr 侧栏即节点卡片行。看板是独立的 `coop-board` Ratatui 插件（[herdr/coop-board](../../../herdr/coop-board/README.zh.md)）：`herdr plugin link <repo>/herdr/coop-board` 后 `herdr plugin pane open --plugin coop.board --entrypoint board` 打开 overlay（q/Esc 关闭）；窗格命令从插件根目录运行，请指向带 `/coop workspace init` 锚点的 workspace 或传显式根路径 —— 详见插件 README 的发现章节。
